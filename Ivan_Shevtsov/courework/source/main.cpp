...
void time_AStar(){
    ...
    Node* start  = new   Node(1.0, 0.0, 0.0);
    Node* target = new   Node(2, INT_MAX, INT_MAX);
    graph1.add_node(start  );
    graph1.add_node(target );


    list_names::list<Node*> prevs;
    prevs.push(start);

    for (int samples = 0; samples < 20; samples++){
        Node* newnode = new   Node(1.0, samples, samples);
        graph1.add_node(newnode);
        graph1.add_edge(start, newnode, gen());
        prevs.push(newnode);
    }
    prevs.pop();

    Node* last;
    for (long testing = 1; testing < 220; testing++) {

        for (unsigned long long i = 20; i < 101; i += 20) {
            for (auto &&node: prevs) {
                for (int samples = 0; samples < 21; samples++) {
                    Node *newnode = new Node(1.0, samples, samples);
                    graph1.add_node(newnode);
                    graph1.add_edge(node, newnode, gen());
                    prevs.push(newnode);
        ...
        graph1.add_edge(last, target, gen());

        auto begin = std::chrono::steady_clock::now();

        graph1.AStarSearch(*start, *target);

        auto end = std::chrono::steady_clock::now();

        auto elapsed_ms = std::chrono::duration_cast<std::chrono::microseconds>(end - begin);

        std::cout << testing << ". Node count: "  << "\n" << graph1.get_size() << "\n" << "time\n";
...
void time_Dijkstra(){
    ...
    for (long testing = 1; testing < 220; testing++) {
        ...
        graph1.naive_Dijkstra_search(start, target, false);
...
void test_binary_heap(){
    binaryHeap_names::BinomialHeap<int> heapBinomial;
    std::random_device rd;
    std::mt19937 gen(rd());
