...
void DijkstraGraph::naive_Dijkstra_search(Node *source, Node *target, bool show_log) {
    ...
    if (show_log){
        ...
        for (auto it = nodes.begin(); it != nodes.end(); ++it){
            std::cout << it.current_node->data->data << "\t";
        }
        std::cout << "\n";
...
bool DijkstraGraph::DijkstraSearch(Node &source, Node &target) {
    // mark source as visited
    source.dist = 0;
    source.visited = true;
...
/*    // MAIN ALGORITHM
    Node* running_node = &source;
    binaryHeap_names::BinomialHeap<Node*> heap;
    while ( !heap.is_empty()){
        running_node = heap.extract_min();
        for (auto it = running_node->edges.begin(); it != running_node->edges.end(); ++it){
            if (heap.is_in_heap(it.current_node->data->target) and )
        ...
        // integration across all nodes and choosing the minimum path
        running_node = find_node_with_min_dist();
    }*/
}
