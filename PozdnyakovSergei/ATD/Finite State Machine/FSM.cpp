...
class State {
public:
    State(std::string name, bool final = false) : name(name), final(final) {}
...
class State_mashine {
public:
    State_mashine(State* start_state) : current_state(start_state) {}
    void add_state(State* state) { states[state->getName()] = state; }
...
State* State::getNextState(char symbol) {
    if (transitions.find(symbol) != transitions.end()) {return transitions[symbol];}
    else {return nullptr;}
}


void State_mashine::add_transition(std::string current_state_name, char symbol, std::string next_state_name) {
...
bool State_mashine::process(std::string input) {
