...
Queue<T, CAPACITY>::Queue(const Queue &other) {
    for (int index = 0; index < CAPACITY; index++) {
        ::new(static_cast<std::aligned_storage_t<sizeof(T), alignof(T)>*> (&queue[index])) T(*std::launder(reinterpret_cast<const T*>(&other.queue[index])));
    }
    front = other.front;
    rear = other.rear;
    empty = other.empty;
...
bool Queue<T, CAPACITY>::push(const T &new_val) {
    if (((front == rear + 1) || (front == 0 && rear + 1 == CAPACITY)) && !empty) return false;
