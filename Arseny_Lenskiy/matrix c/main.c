...
// Резльтаты расчетов для сравнения
...
int is_mat_equal(const Matrix A, const Matrix B){ // Функция для проверки получившихся матриц с ответами
    if(A.cols != B.cols){
        return 0;
    }
    if(A.rows != B.rows){
