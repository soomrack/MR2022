...
struct Node {
    ...
    //
    // Могут спросить, а насколько эта штука эффективна. Взятие по ключу для хештаблиц работает !амортизировано! за O(1).
    // https://habr.com/ru/articles/509220/
    std::unordered_map<int, Edge*> out_edges;
    std::unordered_map<int, Edge*> in_edges;
};

// Будем отдельно хранить ребра в структурах. Это может быть очень полезно, когда ребра сами по себе имеют какие-то свойства:
// например вес, если граф взвешенный; или пропускную способность и текущий поток если мы решаем задачу о потоке.
// https://codeforces.com/blog/entry/78927
struct Edge {
...
private:
    ...
    // А вот ребра придется хранить в обычной map'е. Это связано с тем, что в плюсах не определена функция хеширования для пары.
    // При это на парах есть порядок (то есть определен оператор "<"), поэтому для них работает всякие сбалансированные бинарные деревья.
    // По дефолту С++ для реализации Красно-черное дерево, и почти все операции с деревом работают за O(log n).
    //
    // Также есть маленький бонус, что мы используем map: когда мы проходимся for'ом по ребрам (в функции print_edges например),
    // у нас ключи отсортированы, и поэтому вывод красивый получается, то есть все исходящие ребра для одной вершины идут друг за другом.
    //
