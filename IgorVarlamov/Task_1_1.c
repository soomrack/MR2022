...
const unsigned int years = 20;
const int annual_contrib = 60 * 1000 * 100;
const int first_contrib = 300 * 1000;


typedef struct person                                           // Создание структуры person
{
    const char* name;
...
void init_Alice(person* Alice)                                  // Инициализация объекта структуры Alice
{
    Alice->name = "Alice";
...
void init_Bob(person* Bob)                                      // Инициализация объекта структуры Bob
{
    Bob->name = "Bob";
    Bob->tax = 0.001;
...
void month_pay (person* buyer)                                  // Месячный платеж
{
...
void live_changes_Alice(person* A, const unsigned int year)     // Изменения в доходах/ставке и итд...
{ 
    if (year == 10) 
    {
...
void live_changes_Bob(person* B, const unsigned int year)       // Изменения в доходах/ставке и итд... 
{
    if (year % 5 == 0)
        B->house = (B->house * 3 / 2);
...
void output ( person* client, unsigned int year)                // Вывод в консоль
{
    printf("%s\n",client ->name);
...
void print_report ( person* A,  person* B)                      // Вывод в консоль результатов расчета
{
