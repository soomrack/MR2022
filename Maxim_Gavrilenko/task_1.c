...
int main() {
    ...
    for (char year = 1; year <= 20; year++) {
        for (char month = 1; month <=12; month++)
        {
            ...
            bank_a = (unsigned long long int)((bank_a + revenue - a_utilities) * (1.f+ rate/12));
            bank_b = (unsigned long long int)((bank_b + revenue - b_utilities - month_cont) * (1.f + rate/ 12));
        }
        rubles_a = bank_a / 100;
        cents_a = bank_a % 100;
        rubles_b = bank_b / 100;
