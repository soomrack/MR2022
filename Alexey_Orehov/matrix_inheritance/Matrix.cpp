...
MatrixMemory::MatrixMemory(unsigned int r_num, unsigned int c_num) {
    rows = r_num;
    cols = c_num;
    values = new double[rows * cols];
...
MatrixMemory::MatrixMemory(const MatrixMemory& mat) {
    rows = mat.rows;
    cols = mat.cols;
    values = new double[rows * cols];
    if (!values) throw MatrixException("Error: unable to allocate memory");
...
MatrixMemory::MatrixMemory(MatrixMemory &&mat) noexcept {
    values = mat.values;
    rows = mat.rows;
    cols = mat.cols;
    memory_size = mat.memory_size;
...
std::ostream& operator<<(std::ostream& os, MatrixMemory& mat) {
    os << "Total memory allocated for MatrixMemory class " << mat.get_total_mem() << std::endl;
    return os;
}