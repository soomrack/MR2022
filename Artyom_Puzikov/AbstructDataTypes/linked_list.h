#ifndef ABSTRUCTDATATYPES_LINKED_LIST_H
#define ABSTRUCTDATATYPES_LINKED_LIST_H
...
public:
	...
	T pop_next();
	explicit Node(T new_data){data = new_data; next = nullptr;}
};
...
Node<Type>* LinkedList<Type>::find(Type find_data) {
	Node<Type>* currenrt = head;
	if(currenrt != nullptr) {
		if(currenrt->data == find_data){
			return currenrt;
		}
