...
Matrix empty();
Matrix zero(const Matrix X);
Matrix indentity(const Matrix X);
...
bool operator==(const Matrix &X, const Matrix &Y) {  // работает
...
Matrix operator +(const Matrix &X, const Matrix &Y) {  //работает
...
Matrix operator*(const Matrix &Y, const Matrix &X) {  // работает
    if (Y.cols != X.rows) {
        throw cannot_be_multiplied;
        return empty();
    }
    Matrix res = Matrix(Y.rows, X.cols); //
...
Matrix operator*(const Matrix &X, double k ) {
    Matrix  res = Matrix ( X.rows, X.cols);
...
Matrix operator^(const Matrix &X, const double step){
...
Matrix operator%(const Matrix &X,const double step){
    ...
    Matrix result_exp = Matrix(X.rows, X.cols);
    result_exp = X;
    double factor = 0.5;
...
Matrix  zero (const Matrix X){
    Matrix zero = Matrix (X.rows, X.cols);
...
Matrix indentity(const Matrix X){
    ...
    Matrix one = Matrix (X.rows , X.cols);
    one = zero (X)   ;
...
Matrix Matrix:: pow(const Matrix X, const unsigned int step){
    if ( X.rows != X.cols){
