...
const double EPS_1 = 0.0001;

bool is_triangle(const Matrix<double>& matrix) {
...
bool test_add() {
  ...
  Matrix<double> first_matrix = Matrix<double>(3, 4, values_first);
  Matrix<double> second_matrix = Matrix<double>(3, 4, values_second);
  Matrix<double> result_matrix = Matrix<double>(3, 4, values_result);
...
bool test_multiply() {
  ...
  Matrix<double> first_matrix = Matrix<double>(3, 4, values_first);
  Matrix<double> second_matrix = Matrix<double>(4, 2, values_second);
  Matrix<double> result_matrix = Matrix<double>(3, 2, values_result);
  if (result_matrix == first_matrix * second_matrix) {
    return true;
  }
  return false;
...
bool test_divide_by_scalar() {
  ...
  double values_result[6] = {0.2, -20,
                             0, 40};
  Matrix<double> first_matrix = Matrix<double>(2, 2, values_first);
  Matrix<double> result_matrix = Matrix<double>(2, 2, values_result);
...
bool test_triangle_matrix() {
  double values[9] = {17, 24, 3,
                      51, -5, 7,
                      9, 10, 11};
  Matrix<double> matrix = Matrix<double>(3, 3, values);
...
bool test_transposed() {
  ...
  double values_result[6] = {1, 4,
                             2, 5,
                             3, 6};
  Matrix<double> matrix = Matrix<double>(2, 3, values);
  Matrix<double> resultMatrix = Matrix<double>(3, 2, values_result);
...
bool test_determinant() {
  ...
  if (std::abs(matrix.get_determinant() - determinant) < EPS_1) {
    return true;
  }
  return false;
}
...
bool test_minor() {
  ...
  if (std::abs(matrix.get_minor(1, 1) - determinant) < EPS_1) {
    return true;
  }
  return false;
}
...
bool test_cofactor() {
  ...
  Matrix<double> matrix = Matrix<double>(3, 3, values);
  Matrix<double> result_matrix = Matrix<double>(3, 3, values_result);
...
bool test_exponent () {
    double values[9] =
            {1, 2, 3,
             4, 5, 4,
             3, 2, 1};
    Matrix<double> first_matrix(3, 3, values);
    Matrix<double> second_matrix = first_matrix.multiply(-1);
    Matrix<double> first_result_matrix = first_matrix.get_matrix_exponent(0.00001);
    Matrix<double> second_result_matrix = second_matrix.get_matrix_exponent(0.00001);
...
bool test_exception_add() {
  ...
  Matrix<double> first_matrix = Matrix<double>(3, 3, values_first);
  Matrix<double> second_matrix = Matrix<double>(2, 2, values_second);
  try {
    Matrix<double> result_matrix = first_matrix + second_matrix;
...
bool test_exception_division_by_zero() {
  ...
  Matrix<double> matrix = Matrix<double>(3, 3, values_first);
  try {
    Matrix<double> result_matrix = matrix / 0.0;
  } catch (Matrix_exception& e) {
...
bool test_exception_not_square() {
  ...
  Matrix<double> matrix = Matrix<double>(2, 3, values_first);
  try {
...
bool test_exception_too_high_pow_degree() {
    ...
    Matrix<double> matrix = Matrix<double>(3, 3, values);
    try {
        Matrix<double> result = matrix.pow(101);
    } catch (Matrix_exception& e) {
