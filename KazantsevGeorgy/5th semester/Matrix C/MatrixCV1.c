...
void one_number_filling(Matrix *matrix) {
    for (int i = 0; i < matrix -> cols * matrix -> rows; i++)
        matrix->values[i] = 2;
}
...
Matrix power(Matrix matrix, int n) {
    ...
    if (n > 1) {
        if (rows == cols) {
            ...
            printf("Matrix must be squared. Power is impossible!");
            exit(EXIT_FAILURE);
        }
    }
    return resultmatrix;
...
int main() {
    ...
    double det = determinant(matrix_1);
    printf("determinant raven: %f", det);

    matrix_1 = multiply(matrix_1, matrix_2);
    m_output(matrix_1);

    matrix_1 = transposition(matrix_2);
    m_output(matrix_1);

    matrix_1 = subtraction(matrix_1, matrix_2);  //matrix_1 - matrix_2
    m_output(matrix_1);

    matrix_1 = addition(matrix_1, matrix_2);
    m_output(matrix_1);
