...
public:
    ...
    void del(const unsigned int deleted_idx);
...
template <typename T>
void Dynamic_array<T>::del(const unsigned int deleted_idx) {
    if (deleted_idx > length()) return;

    for (unsigned int idx = deleted_idx; idx < length() - 1; idx++){
        data[idx] = data[idx + 1];
    }
