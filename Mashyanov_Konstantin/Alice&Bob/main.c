...
void init_Alice(struct Holder *Alice) {
    Alice->name = malloc(5);  // память под имя 
    Alice->name = "Alice";
    Alice->account = 1000000 * 100;  // Начальные сбережения
    Alice->deposit_interest = 0.07 / 12.0;  // Месячный процент
...
void init_Bob(struct Holder *Bob) {
    Bob->name = malloc(3);  // Память под имя 
    Bob->name = "Bob"; 
    ...
    Bob->apartments = 7000000*100;  // Боб покупает квартиру
    Bob->mortage_percentage = 0.1/12;  
};

void save_account_history(struct Holder *holder, const year) {
    holder->account_history[year - 1] = holder->account;  // Ф-я, выводящая счёт каждый год
}

void mortgage_calculation(struct Holder *holder) {
    holder->mortage_payment = (holder->apartments - holder->mortage_down_payment) 
    / (12 * 20) * (1.0 + holder->mortage_percentage);  // Ф-я, считающая процент по ипотеке

}

void apartment_grow(struct Holder *holder) {
    holder->apartments *= 1.0001;  // Рост цены квартиры
}

void salary_income(struct Holder *holder) {
    holder->account += holder->salary;  // Подсчёт Зарплаты
}

void utility_payment(struct Holder *holder) {
    holder->account -= holder->utility;  // Подсчёт Коммунальных услуг
}

void rent_payment(struct Holder *holder) {
    holder->account -= holder->rent;  // Подсчёт аренды
}

void mortgage_pay(struct Holder *holder) {
    holder->account -= holder->mortage_payment;  // Подсчёт платы по ипотеке
}
...
void print_comparison(struct Holder *holder1, struct Holder *holder2) {
    ...
    if (holder1->account > holder2->account + holder2->apartments) {
        ...
        printf("Alice and Bob are equally wealthy");  // Вывод-сравнение, кто богаче
